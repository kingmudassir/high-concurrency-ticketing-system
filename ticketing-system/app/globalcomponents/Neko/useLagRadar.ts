...
// Define the shape of the config object for TypeScript
...
function lagRadar(config: LagRadarConfig = {}) {
    ...
    function $svg(tag: string, props: Record<string, string | number> = {}, children: Node[] = []) {
        const el = document.createElementNS(svgns, tag);
        Object.keys(props).forEach((prop) => el.setAttribute(prop, String(props[prop])));
    ...
    function animate() {
        ...
        const arc = $arcs[(frames + framePtr - i) % frames] as SVGPathElement;
        arc.style.fillOpacity = (1 - i / frames).toString();
        }

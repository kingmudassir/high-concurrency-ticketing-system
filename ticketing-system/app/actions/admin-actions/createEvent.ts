...
export async function createEventAction(formData: FormData): Promise<ActionResponse> {
    ...
    try {
        ...
        // ── Derive totalTickets from tiers ────────────────────────────────
        const totalTickets = tiers.reduce((sum, t) => sum + (parseInt(t.capacity) || 0), 0);

        // ── Image ─────────────────────────────────────────────────────────
        // In production: upload to S3/Cloudinary here and get back the URL.
        // For now we skip storing binary data and leave imageUrl null.
